...
export function DetailTable({ rows, selected, targets }: { rows: Row[]; selected: number[]; targets: number[] }) {
  ...
  return <section className="panel"><h2>Detailed results</h2><p>All totals, including impossible outcomes. Click a column heading to sort.</p><div className="table-scroll details"><table><thead><tr>{columns.map(c => <th key={c.key} aria-sort={activeSort === c.key ? ascending ? 'ascending' : 'descending' : 'none'}><button onClick={() => { setSort(c.key); setAscending(activeSort === c.key ? !ascending : false); }}>{c.label} {activeSort === c.key ? ascending ? '↑' : '↓' : '↕'}</button></th>)}</tr></thead><tbody>{ordered.map(r => <tr className={targets.includes(r.total) ? 'highlight' : ''} key={r.total}>{columns.map(c => <td key={c.key}>{c.key === 'total' ? r.total : c.key[0] === 'p' ? percent(value(r,c.key)) : value(r,c.key).toFixed(3)}</td>)}</tr>)}</tbody></table></div></section>;
}
